...
// (Legacy) Trang voucher cũ - giữ lại để tham khảo.
...
function Vouchers({ url }) {
  ...
  const targetUserOptions = useMemo(() => {
    if (form.issueType === "monthly_rank") {
      return [
        ["all", TARGET_USER_LABEL.all],
        ["rank", TARGET_USER_LABEL.rank],
      ];
    }
    if (form.issueType === "new_user") {
      return [["all", TARGET_USER_LABEL.all]];
    }
    if (form.issueType === "birthday") {
      return [["all", TARGET_USER_LABEL.all]];
    }
    return Object.entries(TARGET_USER_LABEL).filter(([value]) => value !== "new");
  }, [form.issueType]);
  ...
  useEffect(() => {
    setForm((prev) => {
      ...
      if (prev.issueType === "monthly_rank" && !["all", "rank"].includes(prev.targetUser)) {
        next = { ...next, targetUser: "rank" };
      }

      if (prev.issueType === "new_user" && prev.targetUser !== "new") {
        next = { ...next, targetUser: "new", targetRank: "" };
      }

      if (prev.issueType === "birthday" && prev.targetUser !== "all") {
        next = { ...next, targetUser: "all", targetRank: "" };
      }
            ...
            <div className="rounded-xl border border-gray-200 p-6 shadow-sm">
              ...
              <div className="mt-3 grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <div className="flex flex-wrap gap-4 p-3">
                    {targetUserOptions.map(([value, label]) => (
